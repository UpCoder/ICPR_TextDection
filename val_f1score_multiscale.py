...
val_img_dir = '/home/give/Game/OCR/data/ICPR/rename/100/image_100'
val_gt_dir = '/home/give/Game/OCR/data/ICPR/rename/100/txt_100'
...
def main(argv=None):
    ...
    with tf.get_default_graph().as_default():
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for im_fn in im_fn_list:
                ...
                im_resized_4, (ratio_h_4, ratio_w_4) = resize_image(im, max_side_len=256, multiple=1.0)
                ...
                if not FLAGS.no_write_images:
                    ...
                    cv2.imwrite(score_map_path, np.asarray(np.squeeze(score_0) * 255.0, np.uint8))
                if len(new_boxes) == 0:
                    continue
    ...
    submit_dir = os.path.join(FLAGS.output_dir, 'txt')
    print 'submit_dir is ', submit_dir
    if not os.path.exists(submit_dir):
        os.makedirs(submit_dir)
    files = glob(FLAGS.output_dir + '/*.txt')
